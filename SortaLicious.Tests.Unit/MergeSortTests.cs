...
namespace SortaLicious.Tests.Unit
{
    ...
    class MergeSortTests
    {
        ...
        public void MergeSort_Integers_SortsAscending(int[] values, int[] sortedValues)
        {
            var sorted = _uut.SortAscending(values.ToList());
            Assert.That(sorted.SequenceEqual(sortedValues), "Resulting sequence does not match expected.");
        }
